...
def lobby_page():
    ...
    with ui.row().classes('w-full min-h-full p-4 gap-8 items-start justify-center flex-wrap'):
        
        # Left Column: Status & Lobby Iterator
        ...
        # Right Column: Class Switcher
        with ui.card().classes('w-full max-w-md bg-[var(--clr-surface-a10)] border border-[var(--clr-surface-a20)] rounded-[32px] p-6 sm:p-8 gap-6 shadow-2xl flex-shrink-0 items-center'):
            ui.label('CLASS SWITCHER').classes('text-lg font-bold text-[var(--clr-primary-a30)] tracking-widest')
            
            CLASSES = [
                'wind_knight', 'verdant_oracle', 'stormblade', 'shield_knight', 
                'marksman', 'heavy_guardian', 'frost_mage', 'beat_performer'
            ]
            
            with ui.row().classes('w-full gap-4'):
                current_class_select = ui.select(CLASSES, label='Current Class').props('outlined dense options-dense behavior=menu').classes('flex-1 bg-[var(--clr-surface-a0)] rounded-xl')
                desired_class_select = ui.select(CLASSES, label='Desired Class').props('outlined dense options-dense behavior=menu').classes('flex-1 bg-[var(--clr-surface-a0)] rounded-xl')
            
            with ui.row().classes('w-full gap-4 items-center bg-[var(--clr-surface-a0)] p-3 rounded-2xl border border-[var(--clr-surface-a20)]'):
                ui.icon('keyboard', color='grey').classes('ml-2')
                ui.label('HOTKEY').classes('text-xs font-bold text-[var(--clr-surface-a50)]')
                class_keybind = ui.input(value='\\').props('borderless dense').classes('flex-1 text-center font-mono font-bold text-[var(--clr-light-a0)] min-w-[50px]')
            
            def toggle_class_switcher(e):
                if e.value:
                    if not current_class_select.value or not desired_class_select.value:
                        ui.notify('Please select both Current and Desired classes.', color='warning')
                        e.sender.value = False
                        return
                    
                    if current_class_select.value == desired_class_select.value:
                        ui.notify('Current and Desired classes must be different.', color='warning')
                        e.sender.value = False
                        return

                    class_switcher.set_config(class_keybind.value, current_class_select.value, desired_class_select.value)
                    if class_switcher.start_listening():
                        ui.notify(f'Class Switcher Active ({class_keybind.value})', color='positive')
                    else:
                        ui.notify('Failed to start Class Switcher', color='negative')
                        e.sender.value = False
                else:
                    class_switcher.stop_listening()
                    ui.notify('Class Switcher Disabled', color='warning')

            with ui.row().classes('w-full justify-between items-center mt-4'):
                ui.label('ENABLE').classes('font-bold text-[var(--clr-light-a0)]')
                ui.switch(on_change=toggle_class_switcher).props('color=cyan keep-color')

        # Spacer for floating dock
        ui.element('div').classes('w-full h-32')
